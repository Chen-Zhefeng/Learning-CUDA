...
#define GPU_MALLOC musaMalloc
#define GPU_FREE musaFree
#define GPU_MEMCPY musaMemcpy
#define GPU_MEMSET musaMemset
...
#define GPU_DEVICE_SYNCHRONIZE musaDeviceSynchronize

// 前向声明：host 侧会调用这些 kernel / launcher
template <typename T>
...
T trace(const std::vector<T>& h_input, size_t rows, size_t cols)
{
  ...
  // -----------------------------
  // Device kernels (trace)
  // -----------------------------
  // 说明：这里使用 block 内归约 + block 间原子加。
  // 由于 trace 的对角元素数量最多 min(rows, cols)，总体开销很小。

  // 设备侧 atomicAdd 泛型封装（仅支持本作业要求的 int / float）
  // 注：MUSA 兼容 CUDA 语法，atomicAdd 可直接使用。
  // 放在函数内会导致重复定义，因此放到文件作用域。
  // （见下方 device 代码区）
  ...
  int query_heads, int kv_heads, int head_dim, bool is_causal)
{
  ...
  // C++11 兼容：用 Launcher 分派 float / half 路径
...
// -----------------------------------------------------------
// Kernel 1: Float 路径（并行算点积 + Thread0 串行累加，保证顺序一致）
// -----------------------------------------------------------
  ...
  bool is_causal)
{
  ...
  // 1) Q 缓存到 Shared Memory（避免重复读全局内存）
  ...
  // Pass 2: SumExp（Thread0 严格按 j 顺序累加，减少浮点顺序差异）
  float sum_exp = 0.0f;
  for (int j_base = 0; j_base < valid_src_len; j_base += blockDim.x)
  {
    int my_j = j_base + tid;
...
// -----------------------------------------------------------
// Kernel 2: Half/通用路径（Online Softmax）
// -----------------------------------------------------------
template <typename T>
  ...
  bool is_causal)
{
  ...
  typedef typename ComputeType<T>::type compute_t;
...
// Host 侧 Launch 分派（避免 if constexpr）
template <typename T>
struct FlashAttentionLauncher
{
...
struct FlashAttentionLauncher<float>
{
  static void launch(const float* d_q, const float* d_k, const float* d_v, float* d_o,
    ...
    int /*threads*/, int blocks)
  {
    // 我在 float 路径下同样采用“并行算点积 + thread0 串行累加”的策略保证顺序一致。
    // 这里我不再死写 256，而是保证 blockDim.x >= head_dim，避免 head_dim>256 时输出维度映射出错。
    int threads_f = 256;
